<?php
    ...
    $query= "SELECT posts.*, COUNT(post_toasts.userid) AS toasts, COUNT(post_burns.userid) AS burns FROM posts, post_toasts, post_burns WHERE post_toasts.postid = posts.id AND post_burns.postid = posts.id AND posts.userid IN (SELECT user_no FROM following WHERE follower_id= :userId) OR posts.userid = :userId GROUP BY posts ORDER BY date DESC LIMIT " . $postsPerPage . " OFFSET " . $position; 
